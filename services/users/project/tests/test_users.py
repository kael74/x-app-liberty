...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn('leandrosimeon@upeu.edu.pe', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json_keys(self):
        ...
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'leandrosimeon@upeu.edu.pe'}),
                content_type='application/json',
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
    ...
    def test_single_user(self):
        ...
        user = add_user('Leandro Añamuro', 'leandrosimeon@upeu.edu.pe')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('Leandro Añamuro', data['data']['username'])
            self.assertIn('leandrosimeon@upeu.edu.pe', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_all_users(self):
        ...
        add_user('Leandro Añamuro', 'leandrosimeon@upeu.edu.pe')
        add_user('cetaceta', 'cetaceta@upeu.edu.pe')
        with self.client:
            ...
            self.assertIn(
                'Leandro Añamuro', data['data']['users'][0]['username'])
            self.assertIn(
                'leandrosimeon@upeu.edu.pe', data['data']['users'][0]['email'])
            self.assertIn(
                'cetaceta', data['data']['users'][1]['username'])
            self.assertIn(
                'cetaceta@upeu.edu.pe', data['data']['users'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_with_users(self):
        ...
        add_user('leandro', 'leandrosimeon@upeu.edu.pe')
        add_user('cetaceta', 'cetaceta@upeu.edu.pe')
        with self.client:
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            ...
            self.assertIn(b'cetaceta', response.data)
